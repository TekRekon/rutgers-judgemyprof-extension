...
if (isCSP) {
  ...
  document.addEventListener("click", function (event) {
    ...
    if (
      event.target.closest("#dijit_form_Button_3") ||
      event.target.closest("#dijit_form_Button_2")
    ) {
      addRatingToInstructorElements(null);
      addWebregLinkToIndexElements(null);
    }
    ...
    if (
      event.target.closest(
        'tbody[dojoattachpoint="scheduleList"] a[dojoattachpoint="scheduleName"]'
      )
    ) {
      addRatingToInstructorElements(null);
      addWebregLinkToIndexElements(null);
...
async function addRatingToInstructorElements(elementWithCourseName) {
  try {
    ...
    const promises = instructorElements.map((instructorElement) =>
      rateInstructorElement(
        instructorElement,
        subjectName,
        elementWithCourseName
      )
    );
 ...
 */
async function rateInstructorElement(
  instructorElement,
  subjectName,
  elementWithCourseName
) {
  ...
  if (isCSP && !instructorElement.querySelector(".jmp-rating-bubble")) {
    ...
    for (let i = 0; i < instructorNames.length; i++) {
      ...
      if (i + 1 < instructorNames.length) {
        initializedBubbles.add(
          instructorNames[i] + " " + instructorNames[i + 1]
        );
        initializedBubbles.add(
          instructorNames[i + 1] + " " + instructorNames[i]
        );
      }
      ...
      if (
        ...
        !currProfSearch.includes(" ") ||
        instructorNames.length % 2 === 0
      ) {
        if (currProfSearch.includes(" ")) {
          initializedBubbles.add(instructorNames[i + 1]);
        }
        populateRatingBubble(
          instructorElement,
          ratingBubbleElem,
          response,
          currProfSearch
...
async function addWebregLinkToIndexElement(indexElement) {
  //get the original height of the index element
  const originalHeight = window.getComputedStyle(indexElement).height;
  //get the year and season from the yearTermElement
  const yearTermElement = document.querySelector(
    "span[name='YearTerm']"
  ).textContent;
  ...
  indexElement.style.height = originalHeight;
  indexElement.style.justifyContent = "center";
  indexElement.style.alignItems = "center";
  //create a webreg link
  const webregLink = document.createElement("a");
  webregLink.classList.add("webreg-link");
  webregLink.href = `http://sims.rutgers.edu/webreg/editSchedule.htm?login=cas&semesterSelection=${numericSeason}${year}&indexList=${index}`;
  webregLink.target = "_blank";
  webregLink.rel = "noopener noreferrer";
  webregLink.textContent = "Webreg";
  indexElement.appendChild(webregLink);
}
...
function extractInstructorElements(elementWithCourseName) {
  ...
  if (isSOC || isWebReg || isOldSOC) {
    unfilteredInstructorElements =
      elementWithCourseName.querySelectorAll(".instructors");
  } else if (isCSP) {
    unfilteredInstructorElements = document.querySelectorAll(
      'td[title="Instructor"]'
    );
    //filter out selected instructor elements not from the current dropdown if a dropdown was clicked
    if (elementWithCourseName) {
      unfilteredInstructorElements = Array.from(
        unfilteredInstructorElements
      ).filter(function (instructorElement) {
  ...
  let filteredInstructorElements = Array.from(
    unfilteredInstructorElements
  ).filter(function (instructorElement) {
    return (
      instructorElement &&
      !instructorElement.querySelector(".jmp-rating-bubble")
    );
  });
...
function extractIndexElements(elementWithCourseName) {
  ...
  //filter out index elements that are empty or already have a rating bubble
  let filteredIndexElements = Array.from(unfilteredIndexElements).filter(
    function (indexElement) {
      return indexElement && !indexElement.querySelector(".webreg-link");
    }
  );
...
function extractSubjectName() {
  ...
  if (isSOC || isWebReg) {
    ...
    if (window.location.href.includes("SelectCourseTab")) {
      let subjectDropdown = document.querySelector(
        "#CourseSearchID select:last-of-type"
      );
...
function extractCourseName(instructorElement, elementWithCourseName) {
  if (isSOC || isOldSOC || isWebReg) {
    ...
    if (window.location.href.includes("SelectCourseTab")) {
      if (
        elementWithCourseName &&
        elementWithCourseName.querySelector(".title")
      ) {
...
function extractInstructorNames(instructorElement) {
  let instructorNames = [];
  if (isSOC || isOldSOC || isWebReg) {
    instructorNames = instructorElement.textContent
      .trim()
      .split(";")
      .map((name) => name.trim());
    for (let i = 0; i < instructorNames.length; i++) {
      let currLastFirst = instructorNames[i]
        .split(",")
        .map((name) => name.trim());
      for (let j = 0; j < currLastFirst.length; j++) {
        if (currLastFirst[j].includes(" ")) {
          //has middle initial
...
function initializeLoadingBubble(instructorElement) {
  ...
  if (isCSP) {
    ...
    ratingBubble.style.fontSize = `${getDynamicFontSize(
...
function populateRatingBubble(
  ...
  if (ratingBubbleElem.textContent.trim() !== "N/A") {
    ...
    let ratingColor =
      ratingBubbleElem.textContent === "X.X"
        ? "lightgray"
        : getRatingColor(ratingNum);
    ratingBubbleElem.style.backgroundColor = ratingColor;
    cardRatingElem.style.backgroundColor = ratingColor;

    cardProfName.textContent =
      response.data.firstName + " " + response.data.lastName;
    cardProfDept.textContent = response.data.department;
    reviewsLink.textContent = response.data.numRatings + " review(s)";
    reviewsLink.href =
      "https://www.ratemyprofessors.com/professor/" + response.data.legacyId;
    reviewsLink.target = "_blank";
    cardDifficultyElem.textContent =
      "Level Of Difficulty: " +
      (response.data.numRatings > 0 ? response.data.avgDifficulty : "N/A");
    cardWouldTakeAgainElem.textContent =
      "Would Take Again: " +
      (response.data.wouldTakeAgainPercent !== -1
        ? String(
            Math.round(
              (response.data.wouldTakeAgainPercent + Number.EPSILON) * 100
            ) / 100
          ) + "%"
        : "N/A");
      ...
      if (event.currentTarget === event.target) {
        if (window.location.href.includes("/csp/")) {
          ...
          popupCard.style.left =
            ratingBubbleElem.offsetLeft - popupCard.offsetWidth + "px";
        }
    ...
    if (!profText.includes(" ")) {
      //add bubble/card warnings if prof first name is unavailable
      ...
      cardWarningMessagePopupElem.textContent =
        "Professor may be inaccurate: only last name provided";
  ...
  while (element && element.parentNode) {
    ...
    if (element.className && element.className.toLowerCase() === className) {
      while (element && element.previousSibling) {
        ...
        if (
          element.className &&
          element.className.toLowerCase() === "metadata hidden"
        ) {
          return element;
        }
      }
...
function getDynamicFontSize(
  ...
  if (length > stepThreshold) {
    const stepsOverThreshold = Math.floor(
      (length - stepThreshold) / stepThreshold
    );
